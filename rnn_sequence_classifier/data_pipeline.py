...
class DataPipeline(object):
  def __init__(self, tfrecord_file_name, batch_size):
    self._batch_size = batch_size
    self._dataset = tf.data.TFRecordDataset(tfrecord_file_name) 
    self._iterator = self._dataset.make_one_shot_iterator()

  def GetBatch(self):
    next_item = self._iterator.get_next()
    return next_item
...
class RNNLM(object):
    def __init__(self,
        ...
        self.learning_rate = tf.train.exponential_decay(
            initial_learning_rate, self.global_step, 150, 0.96, staircase=True)
        self.learning_rate = tf.cond(
                tf.less(self.learning_rate, final_learning_rate),
        ...
        def parse(line):
            # TODO TODO The variables whose name starts with "_" are for debugging. 
            self._line = line
            line_split = tf.string_split([line])
            self._line_split = line_split
        ...
        training_dataset = (
            tf.data.TextLineDataset(self.file_name_train).map(parse).
            shuffle(256).padded_batch(
                self.batch_size, padded_shapes=([None], [None])))
        validation_dataset = (
            tf.data.TextLineDataset(self.file_name_validation).map(parse).
            padded_batch(self.batch_size, padded_shapes=([None], [None])))
        test_dataset = (
            tf.data.TextLineDataset(self.file_name_test).map(parse).batch(1))
        ...
        # Input embedding mat
        self.input_embedding_mat = tf.get_variable(
            "input_embedding_mat", [self.vocab_size, self.num_hidden_units],
            dtype=tf.float32)
        self.input_embedded = tf.nn.embedding_lookup(
            self.input_embedding_mat, self.input_batch)

        # LSTM cell
        cell = tf.contrib.rnn.LSTMCell(self.num_hidden_units, state_is_tuple=True)
        cell = tf.contrib.rnn.DropoutWrapper(
            cell, input_keep_prob=self.dropout_rate)
        cell = tf.contrib.rnn.MultiRNNCell(
            cells=[cell]*self.num_layers, state_is_tuple=True)
        self.cell = cell
        ...
        self.output_embedding_bias = tf.get_variable(
            "output_embedding_bias", [self.vocab_size],
            dtype=tf.float32)
        ...
        loss = tf.nn.sparse_softmax_cross_entropy_with_logits(
               labels=tf.reshape(self.output_batch, [-1]), logits=logits) \
        ...
        gradients = tf.gradients(
            self.loss, params, colocate_gradients_with_ops=True)
    ...
    def batch_train(self, sess, saver):
        ...
        while epoch < self.num_epochs:
            ...
            while True:
                try:
                    (input_batch, _loss, _valid_words, global_step,
                        current_learning_rate, _) = \
                            sess.run(
                                [self.input_batch, self.loss, self.valid_words, self.global_step,
                                self.learning_rate, self.updates],
                                {self.dropout_rate: 0.5})

                    # TODO TODO
                    import pdb; pdb.set_trace()
                    print (sess.run([self._line, self._line_split, self._input_seq, self._output_seq]))
                    # End of TODO TODO

                    train_loss += np.sum(_loss)
                    train_valid_words += _valid_words

                    (_loss, _valid_words, global_step,
                        current_learning_rate, _) = \
                            sess.run(
                                [self.loss, self.valid_words, self.global_step,
                                self.learning_rate, self.updates],
                                {self.dropout_rate: 0.5})
                    train_loss += np.sum(_loss)
                    train_valid_words += _valid_words
                    ...
                    if global_step % self.check_point_step == 0:
                        ...
                        print ("Training Step: {0}, LR: {1}".format(
                            global_step, current_learning_rate))
                        print ("    Training PPL: {0}".format(train_ppl))
