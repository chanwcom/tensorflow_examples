...
# TODO(chanw.com) Why is the reversed dictionary needed?
train_data, valid_data, test_data, vocabulary, reversed_dictionary = load_data()

# TODO(chanw.com) Can't we use the tf.data structure?
        ...
        while True:
            for i in range(self.batch_size):
                if self.current_idx + self.num_steps >= len(self.data):
                    ...
                    self.current_idx = 0
                x[i, :] = self.data[
                    self.current_idx:self.current_idx + self.num_steps]
                temp_y = self.data[
                    self.current_idx + 1:self.current_idx + self.num_steps + 1]
...
model.add(keras.layers.CuDNNLSTM(hidden_size, return_sequences=True))
model.add(keras.layers.CuDNNLSTM(hidden_size, return_sequences=True))
